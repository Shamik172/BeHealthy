...
function YogaCards() {
  ...
  useEffect(() => {
    const fetchAsanas = async () => {
      const query = selectedBodyPart ? `?bodyPart=${selectedBodyPart}` : "";
      const url = `http://localhost:5050/asanas/by-body-part${query}`;
  ...
  return (
    <div className="flex min-h-screen">
      <BodyPartSidebar selectedPart={selectedBodyPart} onSelect={setSelectedBodyPart} />

      <div className="flex-1 p-6">
        <h2 className="text-3xl font-bold text-green-700 mb-6 text-center">
          {selectedBodyPart
            ? `Yoga Asanas for ${selectedBodyPart}`
            : "Yoga for Relaxation & Brain Health"}
        </h2>

        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {asanas.map((pose, index) => {
